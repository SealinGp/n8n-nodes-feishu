...
export default {
	...
	options: [
		{
			displayName: `<a target="_blank" href="${getDocumentationUrl('/document/server-docs/approval-v4/instance/get')}">${WORDING.OpenDocument}</a>`,
			name: 'notice',
			type: 'notice',
			default: '',
		},
		{
			displayName: WORDING.UiInstanceCode,
			name: 'instance_code',
			type: 'string',
			required: true,
			default: '',
